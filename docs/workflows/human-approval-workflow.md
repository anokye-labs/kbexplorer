...
rule that conflation/entity-merge changes are **always** human-approved.

For the CI gates that PRs must satisfy, see
[governance with rulesets](governance-with-rulesets.md). For the broader
...
5. **CI**: the PR's required status checks run (see
   [governance with rulesets](governance-with-rulesets.md)).
